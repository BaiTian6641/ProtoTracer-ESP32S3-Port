...
public:
    StripeMaterial(uint8_t colorCount, ProtoRGBColor* rgbColors, float stripeWidth, float wavePeriod, float waveAmplitude){
        this->colorCount = colorCount;
        this->stripeWidth = stripeWidth;
        this->wavePeriod = wavePeriod;
        this->waveAmplitude = waveAmplitude;

        this->rgbColors = new ProtoRGBColor[colorCount];
        this->baseRGBColors = new ProtoRGBColor[colorCount];
    ...
    ProtoRGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override {
        ...
        ProtoRGBColor rgb = rgbColors[startBox];
