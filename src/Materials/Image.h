...
public:
    ...
    ProtoRGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override {
        ...
        if(x <= 0 || x >= xPixels || y <= 0 || y >= yPixels) return ProtoRGBColor();

        unsigned int pos = data[x + y * xPixels] * 3;

        if (pos > colors - (unsigned int)1) return ProtoRGBColor();

        return ProtoRGBColor(rgbColors[pos], rgbColors[pos + 1], rgbColors[pos + 2]).HueShift(hueAngle);
    }
};
