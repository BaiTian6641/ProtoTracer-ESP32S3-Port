...
public:
    ...
    ProtoRGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override{
        ...
        if (inBounds){
            ...
            ProtoRGBColor gradient;
            
            if (density){
                if (d < 0.0f) return ProtoRGBColor(0,0,0);
                gradient = colorDirection->GetRGB(Vector3D(d, 0, 0), Vector3D(), Vector3D());
                //gradient = ProtoRGBColor::InterpolateColors(ProtoRGBColor(0, 0, 0), ProtoRGBColor(255, 255, 255), (d + 1.0f) / 2.0f);

                if(d > 0.5f) gradient = ProtoRGBColor::InterpolateColors(gradient, ProtoRGBColor(255, 255, 255), (d - 0.5f) * 2.0f);
                else gradient = ProtoRGBColor::InterpolateColors(ProtoRGBColor(0, 0, 0), gradient, d * 2.0f);
            }
            else{
                gradient = colorDirection->GetRGB(Vector3D(ratio, 0, 0), Vector3D(), Vector3D());
                if(magn > 0.5f) gradient = ProtoRGBColor::InterpolateColors(gradient, ProtoRGBColor(255, 255, 255), (magn - 0.5f) * 2.0f);
                else gradient = ProtoRGBColor::InterpolateColors(ProtoRGBColor(0, 0, 0), gradient, magn * 2.0f);
            }
