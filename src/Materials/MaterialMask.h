...
public:
    ...
    ProtoRGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override{
        ...
        if(opacity > 0.97f && isInShape){
            ...
            ProtoRGBColor shape = materialShape->GetRGB(position, normal, uvw);
            ProtoRGBColor outer = materialOuter->GetRGB(position, normal, uvw);

            return ProtoRGBColor::InterpolateColors(outer, shape, Mathematics::Constrain(opacity, 0.0f, 1.0f));
        }
        else{
