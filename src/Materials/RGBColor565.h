...
public:
    ...
    ProtoRGBColor(uint8_t R, uint8_t G, uint8_t B){
        R = (R >> 3) & 0x1F; 
        G = (G >> 2) & 0x3F; 
        B = (B >> 3) & 0x1F;
    ...
    ProtoRGBColor(const ProtoRGBColor& rgbColor){
        this->R = rgbColor.R;
        this->G = rgbColor.G;
        this->B = rgbColor.B;
    }

    ProtoRGBColor(Vector3D color){
        this->R = color.X;
        this->G = color.Y;
        this->B = color.Z;
    }
    ...
    ProtoRGBColor Scale(uint8_t maxBrightness){
        uint8_t sR, sG, sB;
        ...
        return ProtoRGBColor(sR, sG, sB);
    }

    ProtoRGBColor Add(uint8_t value){
        int sR, sG, sB;
    ...
    ProtoRGBColor HueShift(float hueDeg){
        ...
        return ProtoRGBColor(rgbVec.X, rgbVec.Y, rgbVec.Z);
    }

    static ProtoRGBColor InterpolateColors(ProtoRGBColor a, ProtoRGBColor b, float ratio){
        ProtoRGBColor c;
