...
class MenuHandler{
private:
    static esp_timer_handle_t singlebmenutimer;
...
public:
    ...
    static bool Initialize(uint8_t pin, uint16_t holdingTime){//if true, eeprom needs set
        ...
        args.callback = [](void * /*arg*/) { UpdateState(); };
        args.arg = nullptr;
        args.dispatch_method = ESP_TIMER_TASK;
        args.name = "singleb_menu";
        args.skip_unhandled_events = false;

        if (esp_timer_create(&args, &singlebmenutimer) == ESP_OK)
        {
            esp_timer_start_periodic(singlebmenutimer, 1000); // 1 ms
        }
        pinMode(pin, INPUT);
...
template<uint8_t menuCount>
esp_timer_handle_t MenuHandler<menuCount>::singlebmenutimer = nullptr;
